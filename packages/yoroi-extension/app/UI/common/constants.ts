export const YOROI_DEV_REMOTE_CONFIG_URL = 'https://yoroi-config.blinklabs.cloud/dev.json';
export const YOROI_PROD_REMOTE_CONFIG_URL = 'https://yoroi-config.blinklabs.cloud/prod.json';
export const DREP_BANNER_MIN_ADA = 5;
export const STAKEPOOL_MARGIN_FEE_URL =
  'https://help.yoroi-wallet.com/en/article/upcoming-update-to-emurgo-and-yoroi-stake-pools-1giae8b/';
export const RESTORE_WALLET_HELP_URL =
  'https://help.yoroi-wallet.com/en/article/i-have-lost-my-password-how-to-reset-it-17w85zj/';

export enum BannerType {
  MidnightPhase2 = 'midnightPhase2Banner',
  DRep = 'drepBanner',
  BuyAda = 'buyAdaBanner',
  Bring = 'bringBanner',
  Usda = 'usdaBanner',
}

export const SUPPORT_CRISP_CHATBOX_URL = 'https://emurgo.github.io/yoroi-crisp-support/';
export const MIDNIGHT_PHASE2_URL = 'https://www.midnight.gd/';
export const HIDDEN_AMOUNT = '******';
export const CARDANO_CARD_URL = 'https://cardanocard.io/';
