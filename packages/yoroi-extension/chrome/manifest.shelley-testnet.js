...
import { genCSP, getCardanoWalletBackendOrigins } from './constants';
...
export default (isDebug: boolean, shouldInjectConnector: boolean): * =>
  buildManifest({
    ...
    contentSecurityPolicy: genCSP({
      isDev: isDebug,
      additional: {
        'connect-src': [serverToPermission(Servers.Primary), serverToPermission(Servers.Testnet), ...cardanoWalletBackendOrigins],
